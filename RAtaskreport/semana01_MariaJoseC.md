...
- `[ ]` Clonar el repositorio y verificar acceso a `DatosOriginales/Cancelaciones/`

---

### Tarea 1 — Inventario de Cancelaciones

**Script a entregar:** `01_Inventario_Cancelaciones_MJC.R` (o `.py`)
**Referencia:** `1_LimpiezaDatos/Example_Inventario.R`
**Ubicación en repo:** `1_LimpiezaDatos/`

El script debe producir un reporte con:

- `[ ]` **Carga de archivos** — cargar todos los archivos de la carpeta `DatosOriginales/Cancelaciones/`
...
- Cómo está codificada la fecha o período de cancelación (formato exacto)
- Variables que cambian entre semestres
- Clave única de observación (variable simple o compuesta)
- Confirmar que el primer archivo disponible es `Cancelaciones_2009-2S` (no existe 2009-1S)

---

### Tarea 2 — Master Dataset de Personas (Cancelaciones)

**Script a entregar:** `02_masterpersonas_Cancelaciones_MJC.R` (o `.py`)
...
- `[ ]` Iterar sobre **todos** los archivos de Cancelaciones (no solo el más reciente)
...
- `[ ]` Guardar en `DatosArmonizados/keys/MASTER_PERSONAS_CANCELACIONES_PII.csv` (solo Drive, nunca a GitHub)
...
|---|---|---|
| `1_LimpiezaDatos/01_Inventario_Cancelaciones_MJC.[ext]` | Creado | Script de inventario |
| `1_LimpiezaDatos/02_masterpersonas_Cancelaciones_MJC.[ext]` | Creado | Script que genera MASTER_PERSONAS_CANCELACIONES_PII |
| `DatosArmonizados/keys/MASTER_PERSONAS_CANCELACIONES_PII.csv` | Creado | Solo en Drive — nunca a GitHub |
...
**Formato de la fecha/período de cancelación:** (completar — ejemplos de valores reales)
...
**¿El primer archivo disponible es `Cancelaciones_2009-2S`?** (confirmar)

**Personas únicas en MASTER_PERSONAS_CANCELACIONES_PII:** (completar)
...
| `01_Inventario_Cancelaciones_MJC` | |
| `02_masterpersonas_Cancelaciones_MJC` | |
| **Total** | |
